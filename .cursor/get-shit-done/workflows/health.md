...
TASKS_DIR="C:/Projects/studypilot-v2/.cursor/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in C:/Projects/studypilot-v2/.cursor/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf C:/Projects/studypilot-v2/.cursor/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
